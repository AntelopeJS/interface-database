...
export default antelopeKnipConfig({
  entry: [
    ...
    // compiled tree ships in the package, and the modules implementing this
    // interface run it as their conformance suite.
    "src/tests/**/*.test.ts",
    "src/antelope.test.ts",
    // Fixtures for those suites. datasets/index.ts is the barrel a suite can
    // pull the whole set from, and datasets/codec_fields.ts is a type-level
    // regression test for io-ts field codecs (#8) whose only job is to keep
