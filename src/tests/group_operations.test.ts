...
const schema = new Schema<{ [tableName]: Order }>("test-group-operations", {
  [tableName]: Order,
});
...
describe("Group Operations", () => {
  ...
  it(
    "Group by Delivery Type with Average Price",
    GroupByDeliveryTypeWithAveragePrice,
  );
  it(
    "Group by Delivery Type with Weighted Average",
    GroupByDeliveryTypeWithWeightedAverage,
  );
  it(
    "Group by Delivery Type with Sum of Totals",
    GroupByDeliveryTypeWithSumOfTotals,
  );
  it(
    "Group by Category with Complex Calculations",
    GroupByCategoryWithComplexCalculations,
  );
  it(
...
async function GroupByDeliveryTypeWithCount() {
  ...
  const expectedExpressCount = testData.filter(
    (order) => order.deliveryType === "express",
  ).length;
  const expectedStandardCount = testData.filter(
    (order) => order.deliveryType === "standard",
  ).length;
...
async function GroupByPaymentStatusWithMultipleAggregations() {
  ...
  expect(result).to.be.an("array");
  const expectedGroupCount = new Set(testData.map((order) => order.isPaid))
    .size;
  expect(result).to.have.lengthOf(expectedGroupCount);
  ...
  const expectedPaidCount = testData.filter(
    (order) => order.isPaid === true,
  ).length;
  const expectedUnpaidCount = testData.filter(
