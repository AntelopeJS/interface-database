...
async function InnerJoinOrdersWithUsers() {
  ...
  const aliceOrders = result.filter(
    (doc) => doc.customerEmail === "alice@example.com",
  );
  const expectedAliceOrdersCount = ordersData.filter(
    (order) => order.customerEmail === "alice@example.com",
  ).length;
...
async function InnerJoinOrdersWithProducts() {
  ...
  const expectedLaptopOrdersCount = ordersData.filter(
    (order) => order.productSku === "LAPTOP-001",
  ).length;
...
async function MultipleJoins() {
  ...
  const aliceBookOrder = result.find(
    (doc) =>
      doc.customerEmail === "alice@example.com" &&
      doc.productSku === "BOOK-001",
  );
...
async function JoinWithFilter() {
  ...
  const activeUsers = result.filter(
    (doc) => doc.customerEmail === "antoine@example.com",
  );
