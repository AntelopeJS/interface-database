...
describe("Row-Level Single-Table Operations", () => {
  ...
  it(
    "Update with expression (single selection)",
    UpdateWithExpressionOnSingleSelection,
  );
...
async function Avg() {
  ...
  const expectedAvg =
    vehicles.reduce((acc, v) => acc + v.price, 0) / vehicles.length;
...
async function BetweenPlusFilter() {
  ...
  const expected = vehicles.filter(
    (v) => v.price >= -2000 && v.price < 5000 && v.isElectric,
  );
  expect(result).to.have.lengthOf(expected.length);
  result.forEach((doc) => {
    expect(doc.isElectric).to.equal(true);
...
async function UpdateScoped() {
  ...
  const t1Updated = await t1Vehicles
    .filter((doc) => doc.key("car").eq("Peugeot"))
    .run();
...
async function UpdateWithExpressionNumeric() {
  ...
  const updated = await t1Vehicles
    .filter((doc) => doc.key("car").eq("Peugeot"))
    .run();
